...
futaba_cmd_servo gSv_A = futaba_cmd_servo(1, &Sci5, 115200);
futaba_cmd_servo gSv_B = futaba_cmd_servo(2, &Sci5, 115200);
sd_wav gSdWav = sd_wav();
...
void inrof2015::begin(void){
	...
	//Oled.println("%d", Psd0->setI2cAddress(0x50, PD0));
	//while(1);
	...
	//Mpu->setup();
	ColorSens->wakeup();
	ColorSens->startMeasurement();
	
	// ビットマップ表示準備
	OledBmp = new mmcBmpOled(&Oled, &MMC);
...
void inrof2015::step(void){
	...
	if(fExe200ms){	// 10msのより先にやらないと、カラーセンサのI2Cできない(new失敗する)
		step200ms();
		fExe200ms = false;
	}
	if(fExe10ms){
...
void inrof2015::step1ms(void){
	...
	************/
	...
	//SpdTgt_ = 200;
	//AngVelTgt_ = 0.000005*(getLineSensAd(1) - getLineSensAd(2));	// ライントレース
	
	// 状態管理
	// 動作管理
	// 制御 (SpdTgt_とAngVelTgt_を決定する)
	manageState();
	...
	************/
	//SpdTgt_ = 200;
	//AngVelTgt_ = 0.000005*(getLineSensAd(1) - getLineSensAd(2));	
	
	RateLimitSpdTgt->setRate(500, 1000);	// 減速側は強めで。
	RateLimitAngVelTgt->setRate(0.5);
	RateLimitSpdTgt->limitRate(&SpdTgt_);
	RateLimitAngVelTgt->limitRate(&AngVelTgt_);
	
	
	ctrlRun(SpdTgt_, AngVelTgt_, &RTireDutyTgt, &LTireDutyTgt);		// 速度制御
...
void inrof2015::isr1ms(void){
	static uint8_t Cnt200ms;
	static uint8_t Cnt50ms;
	static uint8_t Cnt10ms;
	
	
	//static ctrl_ratelimiter RateLimitSpdTgt(1000.0);			// 100 mm/s/s
	//static ctrl_ratelimiter RateLimitAngVelTgt(2.0);	// 1 rev/s/s
	
	clearFreeCounterOvfFlag();
	uint16_t StartTime = getFreeCounter();
	...
	*/
	...
	// 1ms割り込みの実行時間
	if(isFreeCounterOvf()){	// オーバーフローしてたら
		ExeTime = (0xFFFF - StartTime) + getFreeCounter() + 1;
	}else{
		ExeTime = getFreeCounter() - StartTime;
	}
}

void inrof2015::isr10ms(void){
	int8_t tmp = 0;
	int8_t ack2 = 0;
	// センサ更新要求(通信で取る系)
	// サーボ
	ack2 = ServoF->updatePresentPosition();
	ack2 = ServoR->updatePresentPosition();
	// PSDセンサ
	tmp = PsdLeft->measure();
	tmp = PsdLeftSide->measure();
	tmp = PsdFront->measure();
	tmp = PsdRightSide->measure();
	tmp = PsdRight->measure();
	
	//Mpu->measure();	// 今回は使わない
	//ColorSens->updateSens();	// -> 200msごとに実施
	
}

void inrof2015::isr200ms(void){
	// ボール色更新
	updateBallColor();
...
void inrof2015::updateEncoder(void){
	...
	REncCntDiff_ = EncR->getCntDiff();
	LEncCntDiff_ = EncL->getCntDiff();
}
...
void inrof2015::updateTyre(void){
	...
	// タイヤ角速度[rps]
	RTireSpd_rps_ = -(float)REncCntDiff_/ENC_CPR/GEAR_RATIO/SAMPLETIME_CTRL_S;
	LTireSpd_rps_ =  (float)LEncCntDiff_/ENC_CPR/GEAR_RATIO/SAMPLETIME_CTRL_S;
	
	// タイヤ角度[rev]
	RTirePos_r_ = RTirePos_r_ + (float)REncCntDiff_/ENC_CPR/GEAR_RATIO;
	LTirePos_r_ = LTirePos_r_ - (float)LEncCntDiff_/ENC_CPR/GEAR_RATIO;
...
void inrof2015::updateOdmetry(void){
	...
	CalcOdmetry.REncCntDiff[0] = -REncCntDiff_;
	CalcOdmetry.LEncCntDiff[1] = CalcOdmetry.LEncCntDiff[0];
	CalcOdmetry.LEncCntDiff[0] = LEncCntDiff_;
...
void inrof2015::ctrlRun(float SpdTgt_mmps, float AngVelTgt_rps, int32_t* RTireDutyTgt, int32_t* LTireDutyTgt){
	...
	if(fMotorOut){
		// PID制御
		*RTireDutyTgt = (int32_t)PidCtrlRTire.calc(RTireSpdTgt_rps - RTireSpd_rps_);
		*LTireDutyTgt = (int32_t)PidCtrlLTire.calc(LTireSpdTgt_rps - LTireSpd_rps_);
	}else{
		// モーター出力無効時は、PID制御にリセットをかけ、目標デューティーを0とする。
		PidCtrlRTire.resetStatus(RTireSpdTgt_rps - RTireSpd_rps_);
		PidCtrlLTire.resetStatus(LTireSpdTgt_rps - LTireSpd_rps_);
		*RTireDutyTgt = 0;
		*LTireDutyTgt = 0;
	}
}
...
bool_t inrof2015::manageAction_tansaku(void){
	...
	// 状態遷移(ボール見つけたっぽい)
	if(IsBallFinding){
		ActionState = INROF_ACTION_BALLGET;		// ボール取得ステート
	...
	case INROF_ACTION_TURN:		// 180°回る
		...
		case 1:	//180度旋回
			if(0.05 > fabs(MachinePos.Th - (MachinePosActionManageSub.Th + PI))){
				...
				SpdTgt_ = 0;		// [mmps]
				AngVelTgt_ = 0.2 * ((MachinePosActionManageSub.Th + PI) - MachinePos.Th);	// [rps]
				BallFindEnable = false;
			}
			break;
		}
	...
	case INROF_ACTION_BALLGET:	// ボールがいたので掴まえる
		...
		case 2:		// フロントセンサでボール見つけた。ボールなくなるまで更に回るよ
			if(fabs(MachinePos.Th - MachinePosActionManageSub.Th) > (PI*2/3)){		// 120度以上回ったらやめて探索再開
				...
				SpdTgt_ = 0;		// [mmps]
				AngVelTgt_ = (BallFindTh - MachinePos.Th)/12;	// [rps]
			}else{
				if(BallDir){	// ボールは左方向
					//旋回
					SpdTgt_ = 0;		// [mmps]
		...
		case 4:		// ボールのほう向いた。前後距離合わせるよ
			...
			if((BALLFIND_MAXDISTANCE_FRONT<PsdFront->getDistance()) || (0>PsdFront->getDistance() )){	// ボール無い or 無効値
				...
				SpdTgt_ = 5 * (PsdFront->getDistance() - BALLGET_DISTANCE_FRONT);		// [mmps]
				if(100<SpdTgt_){
					SpdTgt_ = 100;
				}else if(-100>SpdTgt_){
					SpdTgt_ = -100;
		...
		case 6:		// アーム下ろした。前進するよ
			if((BALLGET_DISTANCE_FRONT-20) < (MachinePosActionManageSub - MachinePos)){ 	// 前進完
				...
				SpdTgt_ = 1.0*(BALLGET_DISTANCE_FRONT - (MachinePosActionManageSub - MachinePos));		// [mmps]
				//SpdTgt_ = 30;		// [mmps]
				AngVelTgt_ = 0.0;	// [rps]
			}
			break;
		...
		case 7:		// 前後距離合わせた。アーム上げるよ
			if(StartTimeActionManage + 800 < getTime() /* サーボ動作完 */){ 	// サーボ上げた
		...
		case 8:		// ボールとった！！！！ラインへ戻るよ
			...
			//){ 	// ライン上に戻ってきた
				...
				if((MachinePos.getNormalizeTh() > PI/2) || (MachinePos.getNormalizeTh() < -PI/2)){	// 左向いてる
					SpdTgt_ = -3 * (MachinePosActionManage.X - MachinePos.X);		// [mmps]
				}else{																				// 右向いてる
					SpdTgt_ = 3 * (MachinePosActionManage.X - MachinePos.X);
				}*/
				if(ORIENT_WEST == MachineOrientation){	// 西向いてる
					SpdTgt_ = -3 * (MachinePosActionManage.X - MachinePos.X);		// [mmps]
					...
					SpdTgt_ = -3 * (MachinePosActionManage.Y - MachinePos.Y);		// [mmps]
				}else{											// 北向いてる
					SpdTgt_ = 3 * (MachinePosActionManage.Y - MachinePos.Y);
				}
				AngVelTgt_ = 0.0;	// [rps]
			}
			break;
		case 9:		// 北向く
			if( (fabs(MachinePos.getNormalizeTh()-PI/2) < 0.15) && (StartTimeActionManage + 1000 < getTime())){	// 北向いた。ライントレース移るのでラフでおっけーなはず。
																												// かつ、ボール色センサ起動して1000ms以上経っていること。
				ManageActionTansakuState_BallFind = 10;
				
				//return true;		
			}else{
				SpdTgt_ = 0.0;		// [mmps]
				AngVelTgt_ = 0.2 * (PI/2 - ((MachinePos.getNormalizeTh()< -PI/2)?MachinePos.getNormalizeTh()+2*PI:MachinePos.getNormalizeTh()));	// [rps]
				if(0.2<AngVelTgt_){
					SpdTgt_ = 0.2;
				}else if(-0.2>AngVelTgt_){
					SpdTgt_ = -0.2;
				}
			}
...
// ボール見つけたけど見失った時用ステート
		...
		//	){ 	// ライン上に戻ってきた
				...
				/*
				if((MachinePos.getNormalizeTh() > PI/2) || (MachinePos.getNormalizeTh() < -PI/2)){	// 左向いてる
					SpdTgt_ = -5 * (MachinePosActionManage.X - MachinePos.X);		// [mmps]
				}else{																				// 右向いてる
					SpdTgt_ = 5 * (MachinePosActionManage.X - MachinePos.X);
				}
				*/
				if(ORIENT_WEST == MachineOrientation){	// 西向いてる
					SpdTgt_ = -3 * (MachinePosActionManage.X - MachinePos.X);		// [mmps]
		...
		case 41:	//続・ボール見つけたと思ったけど違った
			...
			if(0.05 > fabs(MachinePosActionManage.Th - MachinePos.Th)){
				...
				SpdTgt_ = 0.0;		// [mmps]
				AngVelTgt_ = 0.15*(MachinePosActionManage.Th - MachinePos.Th);	// [rps]
			}
			break;
		...
		case 3: 	// ボール捨てたから下がりながら右回転する
			if( ((fabs(fabs(MachinePos.Th - (MachinePosActionManageSub.Th))-PI) < 0.1)&&(INROF_LINEDIR_FORWARD != ManageActionLineTraceDir))		// 北(青以外)・東(青)を向く(元の向きの反対を向く)
				...
				if(INROF_BLUE == BallColorFixed){
					SpdTgt_ = 0.0;
					AngVelTgt_ = 0.2 * ( (MachinePosActionManageSub.Th - PI) - MachinePos.Th );  // 右180°回転
					if(0.2<AngVelTgt_){
						SpdTgt_ = 0.2;
					}else if(-0.2>AngVelTgt_){
						SpdTgt_ = -0.2;
...
void inrof2015::setTgtVel_LineTrace(float SpdTgt){
	...
	if(INROF_CROSS_START==Cross){
		...
		if((getLineSensAd(1)<LINESENS_THRESHOLD_CENTER)||(getLineSensAd(2)<LINESENS_THRESHOLD_CENTER)){
			...
			// 左右2つのセンサどちらかライン乗ってる
			AngVelTgt_ = 0.000004*(getLineSensAd(0) - getLineSensAd(3));	// ライントレース
...
void inrof2015::setTgtVel_Turn(float SpdTgt, float Radius){
	...
	this->SpdTgt_ = SpdTgt;							// [mm/s]
	//this->AngVelTgt_ = SpdTgt / Radius / (2*PI) /4 ;	// [rev/s]
	this->AngVelTgt_ = this->Spd / Radius / (2*PI) /2;	// [rev/s]
...
//	uint16_t SamplingFreq = 1000;
	uint16_t SamplingFreq = 1/SAMPLETIME_CTRL_S;
...
//	uint16_t cnt = PCLK/1024/SamplingFreq;
	uint8_t cnt = (uint8_t)(PCLK/1024/SamplingFreq);
	TMR0.TCNT = 0;
