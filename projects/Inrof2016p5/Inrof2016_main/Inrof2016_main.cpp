...
void resetWDT(void);		// WDTクリア(開始)
void stopWDT(void);		// WDT停止
void softwareReset(void);	// ソフトウェアリセット

// WDT設定
#pragma address OFS0=0xffffffc8 
const unsigned long OFS0 = 0xffedff5d; 
//IWDT 16384サイクル 256分周 リセット実行
...
void setup()
{
	...
	//set_new_handler(__heap_chk_fail);
...
void loop(void){
	...
	static bool_t sw[2] = {0, 0};
	
	static bool_t sw0[2] = {0, 0};
	static bool_t sw1[2] = {0, 0};
	static bool_t sw2[2] = {0, 0};
	static bool_t sw3[2] = {0, 0};
	static bool_t SwRmbClean[2] = {0, 0};
	static bool_t SwRmbSpot[2] = {0, 0};
	static bool_t SwRmbDock[2] = {0, 0};
	static bool_t SwRmbClock[2] = {0, 0};
	static bool_t SwRmbSchedule[2] = {0, 0};
	static bool_t SwRmbDay[2] = {0, 0};
	
	static bool_t IsBump[2] = {0, 0};
...
//	Inrof.updateEncoder();
	...
	//if(0){
		...
		Oled.print("PSD R%03d C%03d L%03d\r\n", Inrof.PsdRight->getDistance(), Inrof.PsdCenter->getDistance(), Inrof.PsdLeft->getDistance());
		
		Oled.setCurNewLine(7, 0);
		Oled.print("BattAd=%2.1f", Inrof.getBattVoltage() );
	...
	/*
		Sci0.print("PSD_R(%x) = %03dmm C(%x)= %03dmm L(%x) = %03dmm, ArmEnc=%d(0x%X) iscliff %d spd%f, col%d(hsv%3d), RmbCon%X\r\n", 
	...
	*/
		...
		position *MachinePos;
		MachinePos = Inrof.getMachinePos();
		Sci0.print("UpErr 0x%02x, ArmEnc %3d, MachinePos X%f, Y%f, Th%f, FiedlBallNum%d\r\n", InrofUpper.getErr(), InrofUpper.ArmEnc, MachinePos->X, MachinePos->Y, MachinePos->Th, Inrof.FieldBall);
		*/
		position *MachinePos;
		MachinePos = Inrof.getMachinePos();
		Sci0.print("%08d ArmEnc %3d, MachinePos X%f, Y%f, Th%f, FirstGuru%d, FiedlBallNum%d headnum R%d Y%d B%d ", 
				getTime_ms(), 
				InrofUpper.ArmEnc, 
				MachinePos->X, MachinePos->Y, MachinePos->Th, 
				Inrof.fFirstGuruGuruFin,
				Inrof.FieldBall,
				InrofUpper.numRed(),
		...
		// I2C
		Sci0.print("PSD R %d %d, C %d %d, L %d %d\r\n", 
				Inrof.PsdRight->isComuErr(), Inrof.PsdRight->getDistance(), 
				Inrof.PsdCenter->isComuErr(), Inrof.PsdCenter->getDistance(), 
				Inrof.PsdLeft->isComuErr(), Inrof.PsdLeft->getDistance()
				);
		/*		
		// ジャイロ
		Sci0.print("IMU %d GyroSum:%2.2f, %2.2f, %2.2f Temp:%2.2f \r\n", 
				Inrof.Mpu->isComuErr(),
				//Inrof.Mpu->getAccelX(), Inrof.Mpu->getAccelY(), Inrof.Mpu->getAccelZ(), 
				//Inrof.Mpu->getGyroX(), Inrof.Mpu->getGyroY(), Inrof.Mpu->getGyroZ(),
				Inrof.Mpu->getAeRoll(), Inrof.Mpu->getAePitch(), Inrof.Mpu->getAeYaw(),
				Inrof.Mpu->getTemp()
				);
		*/
	...
	if(LastTime+20 < getTime_ms()){
		Sci0.print("Acc:%1.3f, %1.3f, %1.3f Gyro :%1.3f, %1.3f, %1.3f GyroSum:%2.2f, %2.2f, %2.2f Temp:%2.2f \r\n", 
				Inrof.Mpu->getAccelX(), Inrof.Mpu->getAccelY(), Inrof.Mpu->getAccelZ(), 
				Inrof.Mpu->getGyroX(), Inrof.Mpu->getGyroY(), Inrof.Mpu->getGyroZ(),
				Inrof.Mpu->getAeRoll(), Inrof.Mpu->getAePitch(), Inrof.Mpu->getAeYaw(),
				Inrof.Mpu->getTemp()
	...
	*/
	...
	//	Sci0.print("Button : Clean %02X %02X\r\n", SwRmbClean[0],SwRmbClean[1]);
	//}
	if(SwRmbClean[0] && (!SwRmbClean[1])){	
		Sci0.print("Button : Clean\r\n");
	...
	// Dockボタン
	///if(SwRmbDock[0] != SwRmbDock[1]){	
	///	Sci0.print("Button : Dock %02X %02X\r\n", SwRmbDock[0],SwRmbDock[1]);
	//}
	...
	// Clock
	//if(SwRmbClock[0] != SwRmbClock[1]){	
	//	Sci0.print("Button : Clock %02X %02X\r\n", SwRmbClock[0],SwRmbClock[1]);
	//}
	...
	//	Sci0.print("Button : Schedule %02X %02X\r\n", SwRmbSchedule[0],SwRmbSchedule[1]);
	//}
	if(SwRmbSchedule[0] && (!SwRmbSchedule[1])){
		Sci0.print("Button : Schedule\r\n");
		...
		float Interval = (getTime_ms() - ResetTime) / 1000.0;
		
		Inrof.Mpu->setGyroOffset(Inrof.Mpu->getAeRoll() / Interval, Inrof.Mpu->getAePitch() / Interval, Inrof.Mpu->getAeYaw() / Interval, true);
		Inrof.Mpu->resetCalcAttitudeEstimation();
		
		ResetTime = getTime_ms();
		*/
		
		InrofUpper.enableAction();
		Inrof.reqInitPosture();
	...
	//	Sci0.print("Button : Spot %02X %02X\r\n", SwRmbSpot[0],SwRmbSpot[1]);
	//}
	if(SwRmbSpot[0] && (!SwRmbSpot[1])){
		Sci0.print("Button : Spot\r\n");
