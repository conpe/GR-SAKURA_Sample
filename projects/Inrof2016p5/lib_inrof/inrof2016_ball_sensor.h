...
public: 
	...
	int8_t getGlobalObjPos(position* MachinePos, float* PosXw, float* PosYw);	// 対象物の位置
	int8_t getGlobalObjPos(position* MachinePos, position* ObjPos);	
	//void setMachinePos(position* MachinePos);
	void getGlobalPos(position* MachinePos, float* PosXw, float* PosYw);	// センサ位置
	void getGlobalPos(position* MachinePos, position* SensPos);		// センサ位置
	int16_t getObjDistFromMachine(position* MachinePos); 			//マシン中央からの距離
	
	bool_t isObjectBallArea(position* MachinePos);			// 対象物はボールエリア？
